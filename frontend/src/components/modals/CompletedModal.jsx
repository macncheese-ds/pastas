...
export default function CompletedModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t('completed.title')} size="lg">
      <div className="space-y-6">
          ...
          <div className="flex items-start">
            ...
            <div>
              <p className="text-sm font-medium text-amber-300">
                {t('completed.cycleComplete')}
              </p>
              <p className="text-sm text-amber-400 mt-1">
                {t('completed.removalNote')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-neutral-700 p-4">
          <h4 className="text-sm font-medium text-neutral-300 mb-3">{t('completed.pasteInfo')}</h4>
        ...
        <div className="rounded-lg border border-neutral-700 p-4">
          <h4 className="text-sm font-medium text-neutral-300 mb-3">{t('completed.fullHistory')}</h4>
          <div className="space-y-2 text-sm">
            ...
            {paste.viscosity_datetime && (
              <div className="flex items-center">
                <CheckCircleIcon className="h-4 w-4 text-green-400 mr-2" />
                <span className="text-neutral-400">{t('scanAction.viscosityAt')} ({paste.viscosity_value}):</span>
            ...
            {paste.opened_datetime && (
              <div className="flex items-center">
                <CheckCircleIcon className="h-4 w-4 text-green-400 mr-2" />
                <span className="text-neutral-400">{t('scanAction.openedAt')}</span>
        ...
        {isExpired && (
          <div className="rounded-lg bg-red-900/30 border border-red-700 p-3">
            <p className="text-sm text-red-400">
              ⚠️ {t('completed.expiredNote', { date: formatDateTime(paste.expiration_date) })}
            </p>
          </div>
        )}
          ...
          >
            {isLoading ? (
              <>
                ...
                {t('completed.removing')}
              </>
            ) : (
              <>
                <TrashIcon className="h-4 w-4 mr-2" />
                {t('completed.confirmRemoval')}
              </>
            )}
          </button>
        </div>
