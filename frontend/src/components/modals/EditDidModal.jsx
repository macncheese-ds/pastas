...
export default function EditDidModal({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!trimmedDid) {
      setError(t('editDid.didRequired'));
      return;
    }

    if (trimmedDid === paste.did) {
      setError(t('editDid.didUnchanged'));
      return;
    }
    ...
    try {
      ...
      setError(err.message || t('errors.updateDid'));
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={t('editDid.title')} size="sm">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder={t('editDid.enterDid')}
            autoFocus
            disabled={isSubmitting}
          />
          <p className="mt-1 text-xs text-neutral-400">
            {t('editDid.currentDid')} <span className="text-zinc-400 font-medium">{paste.did}</span>
          </p>
        </div>
